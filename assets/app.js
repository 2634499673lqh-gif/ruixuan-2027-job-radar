...
function render() {
  ...
  const list = jobs.filter((job) => {
    ...
    return text.includes(query) && (priority === "全部优先级" || job.priority === priority) && (track === "全部方向" || job.track === track) && (confidence === "全部可信度" || confidenceOf(job) === confidence) && (city === "全部城市" || job.city.includes(city)) && (state === "全部状态" || (saved[job.id] || "未投递") === state);
  });
